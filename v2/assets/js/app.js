...
(function () {
  ...
  if (lb) {
    ...
    var figs = $$('.gal figure'), idx = 0, lbLastFocus = null;
    var openLb = function (i) {
      if (!lb.classList.contains('on')) lbLastFocus = document.activeElement;
    ...
    figs.forEach(function (f, i) {
      var activate = function () { openLb(i); goal('gallery_open'); };
  ...
  if (brg && mnav) {
    var setMenu = function (open) {
      ...
      brg.setAttribute('aria-label', open ? 'Закрыть меню' : 'Открыть меню');
    };
    brg.addEventListener('click', function () {
      setMenu(!mnav.classList.contains('on'));
    });
    $$('a', mnav).forEach(function (a) { a.addEventListener('click', function () { setMenu(false); }); });
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && mnav.classList.contains('on')) { setMenu(false); brg.focus(); }
    });
  ...
  var mdlDefaults = null;
  if (mdl) {
    mdlDefaults = {
      title: mdl.querySelector('form h3') ? mdl.querySelector('form h3').textContent : '',
      subtitle: mdl.querySelector('form > p') ? mdl.querySelector('form > p').textContent : '',
      button: mdl.querySelector('button[type="submit"] span') ? mdl.querySelector('button[type="submit"] span').textContent : '',
      placeholder: mdl.querySelector('textarea[name="note"]') ? mdl.querySelector('textarea[name="note"]').getAttribute('placeholder') : '',
      source: mdl.querySelector('input[name="source"]') ? mdl.querySelector('input[name="source"]').value : ''
    };
  }

  function setMdlCopy(trigger) {
    if (!mdl || !mdlDefaults) return;
    var context = trigger && trigger.getAttribute('data-modal-context');
    var copy = mdlDefaults;
    if (context === 'materials') {
      copy = {
        title: 'Узнать цену и наличие',
        subtitle: 'Менеджер уточнит список и объём, проверит наличие и назовёт актуальную цену.',
        button: 'Получить цену и наличие',
        placeholder: 'Например: кабель ГОСТ, 200 м',
        source: 'Модальное окно: материалы'
      };
    } else if (context === 'question') {
      copy = {
        title: 'Задать вопрос менеджеру',
        subtitle: 'Оставьте телефон и вопрос. Менеджер ответит в рабочее время.',
        button: 'Получить ответ',
        placeholder: 'Напишите вопрос',
        source: 'Модальное окно: вопрос'
      };
    }
    var title = mdl.querySelector('form h3');
    var subtitle = mdl.querySelector('form > p');
    var button = mdl.querySelector('button[type="submit"] span');
    var note = mdl.querySelector('textarea[name="note"]');
    var source = mdl.querySelector('input[name="source"]');
    if (title) title.textContent = copy.title;
    if (subtitle) subtitle.textContent = copy.subtitle;
    if (button) button.textContent = copy.button;
    if (note) note.setAttribute('placeholder', copy.placeholder);
    if (source) source.value = copy.source;
  }

  function openMdl(trigger) {
    if (!mdl) return;
    setMdlCopy(trigger);
  ...
  $$('[data-modal]').forEach(function (b) {
    b.addEventListener('click', function (e) { e.preventDefault(); openMdl(b); });
  });
  ...
  var RATES = {
    ...
    standart: { 'new': 15000, old: 18000, cosm: null },
    elit:     { 'new': 20000, old: 23000, cosm: null }
  };
  ...
  function calcRemont() {
    ...
    var levelInputs = $$('input[name="lvl"]');
    levelInputs.forEach(function (input) {
      input.disabled = obj === 'cosm' && input.value !== 'econom';
    });
    if (obj === 'cosm' && lvl !== 'econom') {
      var economy = $('input[name="lvl"][value="econom"]');
    ...
    if (noteEl) {
      ...
      noteEl.textContent = (obj === 'cosm')
        ? (short ? 'Мелкий косметический ремонт: от 7 000 ₽ за м². Точную сумму считаем после замера.'
                 : 'Мелкий косметический ремонт: от 7 000 ₽ за м². Точная стоимость зависит от объёма и сложности работ и определяется после осмотра объекта.')
